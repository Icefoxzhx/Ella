...
class EllaNoProximityAgent(Agent):
	...
	def _act(self, obs):
		...
		if not self.no_react and self.last_react_time == self.curr_time:
			...
			retrieved_events = self.e_mem.retrieve("important things to react to", obs["rgb"], self.curr_time, self.pose[:3], 3, recency_w=0.5, relevance_w=0.5, proximity_w=0.0)
	...
	def adjust_schedule(self, react_reason=""):
		...
		if current_schedule_place is not None and current_schedule_place != self.current_place:
			...
			if self.last_adjust_schedule_time is not None and (self.curr_time - self.last_adjust_schedule_time).total_seconds() < 60 * 15:
				...
				return None
			retrieved_events = self.e_mem.retrieve("Things I should consider for adjusting my schedule", None, self.curr_time, self.pose[:3], 5, recency_w=0.5, relevance_w=0.5, proximity_w=0.0)
	...
	def generate_utterance(self, target_name, target_knowledge, target_experience):
		...
		retrieved_events = self.e_mem.retrieve(last_chat, None, self.curr_time, self.pose[:3], 3, recency_w=0.5, relevance_w=0.5, proximity_w=0.0)
