...
from generative_agents.global_methods import *
from generative_agents.persona.prompt_template.gpt_structure import *
from generative_agents.persona.prompt_template.print_prompt import *

debug = False

# ENGINE = "llama-3.1-8B-Instruct-quantized.w8a16"
...
def run_gpt_prompt_wake_up_hour(persona, test_input=None, verbose=False): 
  ...
  gpt_param = {"engine": ENGINE, "max_tokens": 5, 
             "temperature": 0.8, "top_p": 1, "stream": False,
             "frequency_penalty": 0, "presence_penalty": 0, "stop": ["\n"]}
  prompt_template = "generative_agents/persona/prompt_template/v2/wake_up_hour_v1.txt"
...
def run_gpt_prompt_daily_plan(persona, 
  ...
  def create_prompt_input(persona, wake_up_hour, test_input=None):
    ...
    if conditions["physical"] is not None:
      iss += conditions["physical"]
      iss += '\n'
    if conditions["mental"] is not None:
      iss += conditions["mental"]
      iss += '\n'
    ##### debug #####
    # iss += "I have caught a cold, leading to a runny nose, sore throat, and mild fatigue. Physical activity is slightly impaired, and the agent may move more slowly or with reduced energy.\n"
    ##### debug #####
    prompt_input += [iss]
  ...
  prompt_template = "generative_agents/persona/prompt_template/v2/daily_planning_v6.txt"
...
def select_rgb_observations(base_path, obs_indexes):
  total_seconds = 24 * 3600
  observation_interval = 10
  num_observations_needed = total_seconds // observation_interval
  latest_observations = obs_indexes[-num_observations_needed:]
  selected_observations = latest_observations[::360]
  image_paths = [f"{base_path}/rgb_{str(i).zfill(4)}.png" for i in selected_observations]
  return image_paths

def run_gpt_prompt_next_daily_plan(persona, 
                              wake_up_hour,
                              conditions,
                              test_input=None, 
                              verbose=False):
  ...
  def create_prompt_input(persona, wake_up_hour, test_input=None):
    ...
    prompt_input = []
    iss = persona.scratch.get_str_iss()
    if conditions["physical"] != None:
      iss += conditions["physical"]
      iss += '\n'
    if conditions["mental"] != None:
      iss += conditions["mental"]
      iss += '\n'
    prompt_input += [iss]
    prompt_input += [persona.scratch.get_str_lifestyle()]
  ...
  gpt4o_param = {"engine": "gpt-4o", "max_tokens": 500, 
               "temperature": 1, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  prompt_template = "generative_agents/persona/prompt_template/v2/daily_planning_v6.txt"
  prompt_input = create_prompt_input(persona, wake_up_hour, test_input)
  prompt = generate_prompt(prompt_input, prompt_template)
  fail_safe = get_fail_safe()
  image_paths = select_rgb_observations(os.path.join("output", "mit_agents_num_5", "generative_agent", "ego", persona.name), list(range(len(persona.rgb_a_mem.seq_obs))))
  # print("here111")
  # print(image_paths)
  sys_prompt = "You are an intelligent writer given images and instructions."
  user_prompt = "You will be shown a set of images from your egocentric view in the previous day. Summarize the day given those images in first person point of view. Output only the summarization."
  day_summary = GPT4o_request_new(image_paths, sys_prompt, user_prompt, gpt4o_param)
  # print("day_summary:", day_summary)
  gpt4_param = {"engine": "gpt4", "max_tokens": 500, 
               "temperature": 1, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  prompt = day_summary + '\n' + prompt
  output = safe_generate_response(prompt, gpt4_param, 5, fail_safe,
                                   __func_validate, __func_clean_up)
  output = ([f"wake up and complete the morning routine at {wake_up_hour}:00 am"]
              + output)
  
  print("new daily plan output:", output)
  exit()
...
def run_gpt_prompt_generate_hourly_schedule(persona, 
  ...
  # gpt_param = {"engine": ENGINE, "max_tokens": 15, 
  #              "temperature": 0, "top_p": 1, "stream": False,
  #              "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  # prompt_template = "generative_agents/persona/prompt_template/v3_ChatGPT/generate_hourly_schedule_v2.txt" ########
  ...
  gpt_param = {"engine": ENGINE, "max_tokens": 50, 
               "temperature": 0.5, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": ["\n"]}
  prompt_template = "generative_agents/persona/prompt_template/v2/generate_hourly_schedule_v2.txt"
  prompt_input = create_prompt_input(persona, 
                                     curr_hour_str, 
                                     p_f_ds_hourly_org,
                                     hour_str, 
...
def run_gpt_prompt_task_decomp(persona, 
  ...
  gpt_param = {"engine": ENGINE, "max_tokens": 1000, 
             "temperature": 0, "top_p": 1, "stream": False,
             "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  prompt_template = "generative_agents/persona/prompt_template/v2/task_decomp_v3.txt"
  ...
  # print ("IMPORTANT DEBUG")

  # print (prompt_input)
  # print (prompt)
  # print ("run_gpt_prompt_task_decomp:", output)
...
def run_gpt_prompt_next_semantic_location(action_description, persona):
  possible_semantic_locations = persona.s_mem.keys()
  # possible_semantic_locations = ["Robotics Institute", "CMU Computer Science Department", "Wean Hall", "Hamburg Hall", "EDSH/Smith Hall", "Doherty Hall", "Carnegie Tech Radio Club ( W3VC.com )"]
  # possible_semantic_locations = ["Hamburg Hall", "EDSH/Smith Hall"]

  def create_prompt_input(action_description, living_place, dining_places):
    # possible_semantic_locations = {"Robotic Institute"}
    assert living_place is not None
    assert dining_places is not None
    possible_semantic_locations_formatted = [f'"{semantic_location}"' for semantic_location in possible_semantic_locations]
    # print("next target prompt:", f"My living area is {living_place} and my work area is {work_area}. Currently, I want to do this action:{action_description}, what is the most possible location for doing this action in the following list: [{','.join(possible_semantic_locations_formatted)}]. You must only return one location from the list.")
    return f"My living area is {living_place}. My favorite restaurant is {dining_places}. Currently, I want to do this action: {action_description}, what is the most possible location for doing this action in the following list: [{','.join(possible_semantic_locations_formatted)}]. Choose only one location from the list and output it as the only response. Do not provide any explanation, reasoning, or additional text."
  
  def __func_clean_up(gpt_response, prompt):
    # possible_semantic_locations = {"Robotic Institute"}
    # print("gpt_response:", gpt_response)
    # print("possible_semantic_locations:", possible_semantic_locations)
    for semantic_location in possible_semantic_locations:
      if semantic_location in gpt_response:
  ...
  gpt_param = {"engine": ENGINE, "max_tokens": 30, 
               "temperature": 0, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  # prompt_template = "generative_agents/persona/prompt_template/v1/action_location_sector_v1.txt"

  prompt = create_prompt_input(action_description, persona.scratch.living_place, persona.scratch.dining_places)
  # prompt = generate_prompt(prompt_input, prompt_template)
  ...
  if output != "wait":
    return output, persona.s_mem[output]
  else:
    return "wait", None
...
def run_gpt_prompt_action_sector(action_description, 
  ...
  def create_prompt_input(action_description, persona, maze, test_input=None): 
    ...
    prompt_input += [persona.scratch.living_place.split(":")[1]]
    x = f"{act_world}:{persona.scratch.living_place.split(':')[1]}"
  ...
  #              "temperature": 0, "top_p": 1, "stream": False,
  #              "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  # prompt_template = "generative_agents/persona/prompt_template/v3_ChatGPT/action_location_sector_v2.txt" ########
  ...
  if output not in x: 
    ...
    output = persona.scratch.living_place.split(":")[1]
...
def run_gpt_prompt_action_arena(action_description, 
  ...
  gpt_param = {"engine": ENGINE, "max_tokens": 15, 
               "temperature": 0, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  prompt_template = "generative_agents/persona/prompt_template/v1/action_location_object_vMar11.txt"
...
def run_gpt_prompt_action_game_object(action_description, 
  ...
  gpt_param = {"engine": ENGINE, "max_tokens": 15, 
               "temperature": 0, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  prompt_template = "generative_agents/persona/prompt_template/v1/action_object_v2.txt"
...
def run_gpt_prompt_pronunciatio(action_description, persona, verbose=False): 
  ...
  gpt_param = {"engine": ENGINE, "max_tokens": 15, 
               "temperature": 0, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  prompt_template = "generative_agents/persona/prompt_template/v3_ChatGPT/generate_pronunciatio_v1.txt" ########
  prompt_input = create_prompt_input(action_description)  ########
  prompt = generate_prompt(prompt_input, prompt_template)
  example_output = "🛁🧖‍♀️" ########
  ...
  #              "temperature": 0, "top_p": 1, "stream": False,
  #              "frequency_penalty": 0, "presence_penalty": 0, "stop": ["\n"]}
  # prompt_template = "generative_agents/persona/prompt_template/v2/generate_pronunciatio_v1.txt"
  # prompt_input = create_prompt_input(action_description)
...
def run_gpt_prompt_event_triple(action_description, persona, verbose=False): 
  ...
  #              "temperature": 0, "top_p": 1, "stream": False,
  #              "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  # prompt_template = "generative_agents/persona/prompt_template/v3_ChatGPT/generate_event_triple_v1.txt" ########
  # prompt_input = create_prompt_input(action_description, persona)  ########
  # prompt = generate_prompt(prompt_input, prompt_template)
  ...
  gpt_param = {"engine": ENGINE, "max_tokens": 30, 
               "temperature": 0, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": ["\n"]}
  prompt_template = "generative_agents/persona/prompt_template/v2/generate_event_triple_v1.txt"
  prompt_input = create_prompt_input(action_description, persona)
  prompt = generate_prompt(prompt_input, prompt_template)
  fail_safe = get_fail_safe(persona) ########
  output = safe_generate_response(prompt, gpt_param, 5, fail_safe,
...
def run_gpt_prompt_act_obj_desc(act_game_object, act_desp, persona, verbose=False): 
  ...
  gpt_param = {"engine": ENGINE, "max_tokens": 15, 
               "temperature": 0, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  prompt_template = "generative_agents/persona/prompt_template/v3_ChatGPT/generate_obj_event_v1.txt" ########
  ...
  #              "temperature": 0, "top_p": 1, "stream": False,
  #              "frequency_penalty": 0, "presence_penalty": 0, "stop": ["\n"]}
  # prompt_template = "generative_agents/persona/prompt_template/v2/generate_obj_event_v1.txt"
  # prompt_input = create_prompt_input(act_game_object, act_desp, persona)
  # prompt = generate_prompt(prompt_input, prompt_template)
  # fail_safe = get_fail_safe(act_game_object)
  # output = safe_generate_response(prompt, gpt_param, 5, fail_safe,
...
def run_gpt_prompt_new_decomp_schedule(persona, 
  ...
  gpt_param = {"engine": ENGINE, "max_tokens": 1000, 
               "temperature": 0, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  prompt_template = "generative_agents/persona/prompt_template/v2/new_decomp_schedule_v1.txt"
  prompt_input = create_prompt_input(persona, 
                                     main_act_dur, 
                                     truncated_act_dur, 
                                     start_time_hour,
...
def run_gpt_prompt_decide_to_talk(persona, target_persona, retrieved, test_input=None, 
  ...
  def create_prompt_input(init_persona, target_persona, retrieved, 
                          test_input=None): 
    last_chat = init_persona.rgb_a_mem.get_last_chat(target_persona.name)
    last_chatted_time = ""
    last_chat_about = ""
    if last_chat: 
  ...
  gpt_param = {"engine": ENGINE, "max_tokens": 20, 
               "temperature": 0, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  prompt_template = "generative_agents/persona/prompt_template/v2/decide_to_talk_v2.txt"
...
def run_gpt_prompt_decide_to_react(persona, target_persona, retrieved,test_input=None, 
  ...
  gpt_param = {"engine": ENGINE, "max_tokens": 20, 
               "temperature": 0, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  prompt_template = "generative_agents/persona/prompt_template/v2/decide_to_react_v1.txt"
  prompt_input = create_prompt_input(persona, target_persona, retrieved,
                                     test_input)
  prompt = generate_prompt(prompt_input, prompt_template)
...
def run_gpt_prompt_decide_to_react_event(persona, observation, retrieved,test_input=None, 
                                       verbose=False): 
  def create_prompt_input(init_persona, observation, retrieved, 
                          test_input=None): 
  ...
  gpt_param = {"engine": ENGINE, "max_tokens": 20, 
               "temperature": 0, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  prompt_template = "generative_agents/persona/prompt_template/v2/decide_to_react_event_v1.txt"
  prompt_input = create_prompt_input(persona, observation, retrieved,
                                     test_input)
  prompt = generate_prompt(prompt_input, prompt_template)
...
def run_gpt_prompt_create_conversation(persona, target_persona, curr_loc,
  ...
  gpt_param = {"engine": ENGINE, "max_tokens": 1000, 
               ...
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  prompt_template = "generative_agents/persona/prompt_template/v2/create_conversation_v2.txt"
...
def run_gpt_prompt_summarize_conversation(persona, conversation, test_input=None, verbose=False): 
  ...
  gpt_param = {"engine": ENGINE, "max_tokens": 15, 
               "temperature": 0, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  prompt_template = "generative_agents/persona/prompt_template/v3_ChatGPT/summarize_conversation_v1.txt" ########
  ...
  #              "temperature": 0, "top_p": 1, "stream": False,
  #              "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  # prompt_template = "generative_agents/persona/prompt_template/v2/summarize_conversation_v1.txt"
  # prompt_input = create_prompt_input(conversation, test_input)
  # prompt = generate_prompt(prompt_input, prompt_template)
...
def run_gpt_prompt_extract_keywords(persona, description, test_input=None, verbose=False): 
  ...
  gpt_param = {"engine": ENGINE, "max_tokens": 50, 
               "temperature": 0, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  prompt_template = "generative_agents/persona/prompt_template/v2/get_keywords_v1.txt"
...
def run_gpt_prompt_keyword_to_thoughts(persona, keyword, concept_summary, test_input=None, verbose=False): 
  ...
  gpt_param = {"engine": ENGINE, "max_tokens": 40, 
               "temperature": 0.7, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  prompt_template = "generative_agents/persona/prompt_template/v2/keyword_to_thoughts_v1.txt"
...
def run_gpt_prompt_convo_to_thoughts(persona, 
  ...
  gpt_param = {"engine": ENGINE, "max_tokens": 40, 
               "temperature": 0.7, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  prompt_template = "generative_agents/persona/prompt_template/v2/convo_to_thoughts_v1.txt"
...
def run_gpt_prompt_event_poignancy(persona, event_description, test_input=None, verbose=False): 
  ...
  gpt_param = {"engine": ENGINE, "max_tokens": 15, 
               "temperature": 0, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  prompt_template = "generative_agents/persona/prompt_template/v3_ChatGPT/poignancy_event_v1.txt" ########
  ...
  # gpt_param = {"engine": ENGINE, "max_tokens": 3, 
  #              "temperature": 0, "top_p": 1, "stream": False,
  #              "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  # prompt_template = "generative_agents/persona/prompt_template/v2/poignancy_event_v1.txt"
  # prompt_input = create_prompt_input(persona, event_description)
  # prompt = generate_prompt(prompt_input, prompt_template)
...
def run_gpt_prompt_thought_poignancy(persona, event_description, test_input=None, verbose=False): 
  ...
  gpt_param = {"engine": ENGINE, "max_tokens": 15, 
               "temperature": 0, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  prompt_template = "generative_agents/persona/prompt_template/v3_ChatGPT/poignancy_thought_v1.txt" ########
  prompt_input = create_prompt_input(persona, event_description)  ########
  prompt = generate_prompt(prompt_input, prompt_template)
  example_output = "5" ########
  special_instruction = "The output should ONLY contain ONE integer value on the scale of 1 to 10." ########
  ...
  #              "temperature": 0, "top_p": 1, "stream": False,
  #              "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  # prompt_template = "generative_agents/persona/prompt_template/v2/poignancy_thought_v1.txt"
  # prompt_input = create_prompt_input(persona, event_description)
  # prompt = generate_prompt(prompt_input, prompt_template)
...
def run_gpt_prompt_chat_poignancy(persona, event_description, test_input=None, verbose=False): 
  ...
  gpt_param = {"engine": ENGINE, "max_tokens": 15, 
               "temperature": 0, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  prompt_template = "generative_agents/persona/prompt_template/v3_ChatGPT/poignancy_chat_v1.txt" ########
  prompt_input = create_prompt_input(persona, event_description)  ########
  prompt = generate_prompt(prompt_input, prompt_template)
  example_output = "5" ########
  special_instruction = "The output should ONLY contain ONE integer value on the scale of 1 to 10." ########
  ...
  #              "temperature": 0, "top_p": 1, "stream": False,
  #              "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  # prompt_template = "generative_agents/persona/prompt_template/v2/poignancy_chat_v1.txt"
  # prompt_input = create_prompt_input(persona, event_description)
  # prompt = generate_prompt(prompt_input, prompt_template)
...
def run_gpt_prompt_focal_pt(persona, statements, n, test_input=None, verbose=False): 
  ...
  gpt_param = {"engine": ENGINE, "max_tokens": 15, 
               "temperature": 0, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  prompt_template = "generative_agents/persona/prompt_template/v3_ChatGPT/generate_focal_pt_v1.txt" ########
  ...
  gpt_param = {"engine": ENGINE, "max_tokens": 150, 
               "temperature": 0, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  prompt_template = "generative_agents/persona/prompt_template/v2/generate_focal_pt_v1.txt"
  prompt_input = create_prompt_input(persona, statements, n)
  prompt = generate_prompt(prompt_input, prompt_template)
...
def run_gpt_prompt_insight_and_guidance(persona, statements, n, test_input=None, verbose=False): 
  ...
  gpt_param = {"engine": ENGINE, "max_tokens": 150, 
               "temperature": 0.5, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  prompt_template = "generative_agents/persona/prompt_template/v2/insight_and_evidence_v1.txt"
  prompt_input = create_prompt_input(persona, statements, n)
  prompt = generate_prompt(prompt_input, prompt_template)
...
def run_gpt_prompt_agent_chat_summarize_ideas(persona, target_persona, statements, curr_context, test_input=None, verbose=False): 
  ...
  gpt_param = {"engine": ENGINE, "max_tokens": 15, 
               "temperature": 0, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  prompt_template = "generative_agents/persona/prompt_template/v3_ChatGPT/summarize_chat_ideas_v1.txt" ########
  ...
  #              "temperature": 0.5, "top_p": 1, "stream": False,
  #              "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  # prompt_template = "generative_agents/persona/prompt_template/v2/summarize_chat_ideas_v1.txt"
  # prompt_input = create_prompt_input(persona, target_persona, statements, curr_context)
  # prompt = generate_prompt(prompt_input, prompt_template)
...
def run_gpt_prompt_agent_chat_summarize_relationship(persona, target_persona, statements, test_input=None, verbose=False): 
  ...
  gpt_param = {"engine": ENGINE, "max_tokens": 15, 
               "temperature": 0, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  prompt_template = "generative_agents/persona/prompt_template/v3_ChatGPT/summarize_chat_relationship_v2.txt" ########
  ...
  #              "temperature": 0.5, "top_p": 1, "stream": False,
  #              "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  # prompt_template = "generative_agents/persona/prompt_template/v2/summarize_chat_relationship_v1.txt"
  # prompt_input = create_prompt_input(persona, target_persona, statements)
  # prompt = generate_prompt(prompt_input, prompt_template)
...
def run_gpt_prompt_agent_chat(maze, persona, target_persona,
  ...
  gpt_param = {"engine": ENGINE, "max_tokens": 15, 
               "temperature": 0, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  prompt_template = "generative_agents/persona/prompt_template/v3_ChatGPT/agent_chat_v1.txt" ########
  ...
  # gpt_param = {"engine": ENGINE, "max_tokens": 2000, 
  #              "temperature": 0.7, "top_p": 1, "stream": False,
  #              "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  # prompt_template = "generative_agents/persona/prompt_template/v2/agent_chat_v1.txt"
  # prompt_input = create_prompt_input(persona, target_persona, curr_context, init_summ_idea, target_summ_idea)
  # prompt = generate_prompt(prompt_input, prompt_template)
...
def run_gpt_prompt_summarize_ideas(persona, statements, question, test_input=None, verbose=False): 
  ...
  gpt_param = {"engine": ENGINE, "max_tokens": 15, 
               "temperature": 0, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  prompt_template = "generative_agents/persona/prompt_template/v3_ChatGPT/summarize_ideas_v1.txt" ########
  ...
  #              "temperature": 0.5, "top_p": 1, "stream": False,
  #              "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  # prompt_template = "generative_agents/persona/prompt_template/v2/summarize_ideas_v1.txt"
  # prompt_input = create_prompt_input(persona, statements, question)
  # prompt = generate_prompt(prompt_input, prompt_template)
...
def run_gpt_prompt_generate_next_convo_line(persona, interlocutor_desc, prev_convo, retrieved_summary, test_input=None, verbose=False): 
  ...
  #              "temperature": 0, "top_p": 1, "stream": False,
  #              "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  # prompt_template = "generative_agents/persona/prompt_template/v3_ChatGPT/generate_next_convo_line_v1.txt" ########
  ...
  gpt_param = {"engine": ENGINE, "max_tokens": 250, 
               "temperature": 1, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  prompt_template = "generative_agents/persona/prompt_template/v2/generate_next_convo_line_v1.txt"
  prompt_input = create_prompt_input(persona, interlocutor_desc, prev_convo, retrieved_summary)
  prompt = generate_prompt(prompt_input, prompt_template)
...
def run_gpt_prompt_generate_whisper_inner_thought(persona, whisper, test_input=None, verbose=False): 
  ...
  gpt_param = {"engine": ENGINE, "max_tokens": 50, 
               "temperature": 0, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  prompt_template = "generative_agents/persona/prompt_template/v2/whisper_inner_thought_v1.txt"
...
def run_gpt_prompt_planning_thought_on_convo(persona, all_utt, test_input=None, verbose=False): 
  ...
  gpt_param = {"engine": ENGINE, "max_tokens": 50, 
               "temperature": 0, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  prompt_template = "generative_agents/persona/prompt_template/v2/planning_thought_on_convo_v1.txt"
...
def run_gpt_prompt_memo_on_convo(persona, all_utt, test_input=None, verbose=False): 
  ...
  gpt_param = {"engine": ENGINE, "max_tokens": 15, 
               "temperature": 0, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  prompt_template = "generative_agents/persona/prompt_template/v3_ChatGPT/memo_on_convo_v1.txt" ########
  prompt_input = create_prompt_input(persona, all_utt)  ########
  prompt = generate_prompt(prompt_input, prompt_template)
  ...
  gpt_param = {"engine": ENGINE, "max_tokens": 50, 
               "temperature": 0, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  prompt_template = "generative_agents/persona/prompt_template/v2/memo_on_convo_v1.txt"
  prompt_input = create_prompt_input(persona, all_utt)
  prompt = generate_prompt(prompt_input, prompt_template)
...
def run_gpt_generate_safety_score(persona, comment, test_input=None, verbose=False): 
  ...
  print ("11")
  prompt_template = "generative_agents/persona/prompt_template/safety/anthromorphosization_v1.txt" 
...
def run_gpt_generate_iterative_chat_utt(init_persona, target_persona, retrieved, curr_context, curr_chat, test_input=None, verbose=False): 
  def create_prompt_input(init_persona, target_persona, retrieved, curr_context, curr_chat, test_input=None):
    ...
    prev_convo_insert = "\n"
    if persona.rgb_a_mem.seq_chat: 
      for i in persona.rgb_a_mem.seq_chat: 
        if i.object == target_persona.scratch.name: 
          v1 = int((persona.scratch.curr_time - i.created).total_seconds()/60)
          prev_convo_insert += f'{str(v1)} minutes ago, {persona.scratch.name} and {target_persona.scratch.name} were already {i.description} This context takes place after that conversation.'
          break
    if prev_convo_insert == "\n": 
      prev_convo_insert = ""
    if persona.rgb_a_mem.seq_chat: 
      if int((persona.scratch.curr_time - persona.rgb_a_mem.seq_chat[-1].created).total_seconds()/60) > 480: 
        prev_convo_insert = ""
    print (prev_convo_insert)

    curr_location = "" # we dont have access to the maze
  ...
  print ("11")
  prompt_template = "generative_agents/persona/prompt_template/v3_ChatGPT/iterative_convo_v1.txt" 
